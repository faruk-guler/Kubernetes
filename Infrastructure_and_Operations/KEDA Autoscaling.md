...
## Prometheus Metrikleri (HTTP İstek Hızı)
...
## RabbitMQ Kuyruk Uzunluğu

RabbitMQ kuyruğundaki bekleyen mesaj sayısına (Queue Length) göre çalışanlarımızı (workers) ölçeklendirmek istiyoruz. Güvenli bir bağlantı için `TriggerAuthentication` kullanarak kimlik bilgilerini bir Secret'tan çekeceğiz.
...
spec:
  ...
  minReplicaCount: 0          # İş yoksa pod sayısını 0'a indir (maliyet tasarrufu)
  maxReplicaCount: 15
  cooldownPeriod: 120
  pollingInterval: 10
  triggers:
  - type: rabbitmq
    metadata:
      ...
      mode: QueueLength
      value: "20"             # Pod başına max 20 mesaj
    authenticationRef:
      name: rabbitmq-auth
---
# RabbitMQ Kimlik Doğrulama
apiVersion: keda.sh/v1alpha1
kind: TriggerAuthentication
metadata:
  name: rabbitmq-auth
...
spec:
  ...
  - parameter: host
    ...
    key: connection-string    # Örnek: amqp://user:password@rabbitmq-service:5672/vhost
```
