...
## 3. Pod ve Konteyner Seviyesinde Güvenlik Ayrımı

Bir pod içerisinde `securityContext` iki farklı seviyede tanımlanabilir:

```yaml
spec:
  # 1. Pod Seviyesi: Pod içindeki TÜM konteynerlere kalıtılan kullanıcı/grup kimlikleri
  securityContext:
    runAsUser: 10001
    runAsGroup: 10001
    runAsNonRoot: true
  ...
  containers:
    - name: secure-app
      image: my-app:v1.0.0
      # 2. Konteyner Seviyesi: Sadece bu konteynere özel katı çekirdek kısıtlamaları
      securityContext:
        allowPrivilegeEscalation: false # Sudo/setuid ile yetki yükseltmeyi engelle
        readOnlyRootFilesystem: true    # Dosya sistemini salt-okunur yap
        capabilities:
          drop: ["ALL"]                 # Tüm Linux çekirdek yetkilerini sıfırla
        seccompProfile:
          type: RuntimeDefault          # Çekirdek sistem çağrılarını kısıtla
```
...
*(Eski Kubernetes sürümlerinde `container.apparmor.security.beta.kubernetes.io/<container>: runtime/default` annotation'ı kullanılırken, Kubernetes 1.30+ ile doğrudan `securityContext.appArmorProfile` GA standardı getirilmiştir).*
