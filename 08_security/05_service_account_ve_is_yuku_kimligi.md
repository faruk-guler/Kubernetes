...
### Bulut Sağlayıcıları ve Eşleştirme Belirteçleri (Annotations)

Her bulut sağlayıcısı, ServiceAccount nesnesine eklenen özel bir `annotation` üzerinden OIDC token değişimini tetikler:

| Bulut Sağlayıcı | Mekanizma | ServiceAccount Belirteci (Annotation) |
| :--- | :--- | :--- |
| **AWS** | IRSA (IAM Roles for Service Accounts) | `eks.amazonaws.com/role-arn: "arn:aws:iam::<HESAP_ID>:role/<ROL_ADI>"` |
| **Google Cloud (GKE)** | GKE Workload Identity | `iam.gke.io/gcp-service-account: "<GSA_ADI>@<PROJE_ID>.iam.gserviceaccount.com"` |
| **Microsoft Azure (AKS)** | Azure Workload Identity | `azure.workload.identity/client-id: "<AZURE_CLIENT_ID>"` |

### Örnek Bulut Entegre ServiceAccount Tanımı
...
metadata:
  ...
  namespace: production
  annotations:
    # Kullanılan bulut sağlayıcısına uygun rol/kimlik belirtilir (Örn: AWS IRSA):
