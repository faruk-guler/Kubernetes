...
## 🛠️ Pratik Otomasyon: Küme Güvenlik ve Sertifika Denetim Araçları

Kümenizdeki güvenlik zafiyetlerini ve sertifika süre sonlarını otomatik olarak denetlemek için aşağıdaki betikleri kullanabilirsiniz:

### 1. Küme İçi Güvenlik Riskleri Taraması

Ayrıcalıklı (privileged) pod'lar, root ile koşan konteynerler, `hostNetwork` / `hostPID` kullanan iş yükleri ve default ServiceAccount yetkilerini tarayan analiz betiği:

```bash
bash ../scripts/audit_security_risks.sh
```

> [!TIP]
> Bu betik [`scripts/audit_security_risks.sh`](../scripts/audit_security_risks.sh) dosyasında yer alır ve küme genelinde güvenlik durumunu puanlar.

### 2. TLS Sertifika Geçerlilik Süresi Denetimi

API Server, etcd, Kubelet ve Control Plane sertifikalarının kaç gün sonra biteceğini önceden tespit etmek için:

```bash
bash ../scripts/audit_cert_expirations.sh
```

> [!TIP]
> Bu betik [`scripts/audit_cert_expirations.sh`](../scripts/audit_cert_expirations.sh) dosyasında yer alır. Süresi 30 günden az kalan sertifikaları kırmızı renkle uyarır.
