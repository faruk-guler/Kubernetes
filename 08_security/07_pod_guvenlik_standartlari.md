...
## 7. Pod Security Standards ve Politika Motorları (Kyverno/Gatekeeper)

Kubernetes yerleşik Pod Security Admission (PSA) mekanizması, küme genelinde katı bir güvenlik taban çizgisi oluşturur; ancak ince taneli denetimler için politika motorları ile birlikte kullanılır:

* **PSA (Yerleşik):** API sunucusu çekirdeğinde çalışır; `Privileged`, `Baseline` ve `Restricted` profilleriyle podları namespace etiketlerine göre filtreler (sıfır gecikme).
* **Kyverno / OPA Gatekeeper (Ekosistem):** PSA'nın sunmadığı dinamik yamalama (mutate), kurumsal etiket kontrolleri ve özel istisna kurallarını yönetir.

*(Kyverno ile kural doğrulama, otomatik enjeksiyon ve CEL politikaları için bkz: **Bölüm 10 — Kyverno ile Kubernetes Politika Yönetimi**).*
