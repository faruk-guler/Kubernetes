...
Kubernetes 1.30+ sürümlerinde yerel `securityContext.appArmorProfile` alanı üzerinden profil atanır:

```yaml
apiVersion: v1
kind: Pod
metadata:
  name: secured-spring-app
  namespace: production
spec:
  securityContext:
    appArmorProfile:
      type: Localhost
      localhostProfile: k8s-apparmor-demo-deny-write
  containers:
    - name: api-container
...
kind: Policy
rules:
  # Gizli bilgileri loglamada sadece metadata düzeyinde kaydet
  - level: Metadata
    resources:
      - group: ""
        resources: ["secrets", "configmaps"]
  # Kritik kontrol düzlemi nesnelerinde tam istek/yanıtı kaydet
  ...
  # Kubelet ve sistem okumalarını yoksay
  - level: None
