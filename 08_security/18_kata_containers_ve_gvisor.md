...
metadata:
  ...
  namespace: default
spec:
  runtimeClassName: gvisor # veya kata-qemu
  containers:
    - name: untrusted-app
      image: alpine:latest
...
```yaml
# Kyverno ile Güvenli Sandbox Runtime Zorunluluğu Kuralı:
rules:
  - name: enforce-gvisor-runtime
    match:
      resources:
        kinds: ["Pod"]
        namespaces: ["untrusted-workloads"]
    validate:
      message: "untrusted-workloads alanında standart runc yerine 'gvisor' veya 'kata' zorunludur!"
      pattern:
        spec:
