...
Mutating webhook yapılandırması `kind: MutatingWebhookConfiguration` dışında Validating webhook ile aynı temel alanları paylaşır; ancak en kritik fark yanıtın içeriğindedir. Webhook servisi nesneyi doğrudan değiştirmek yerine bir **RFC 6902 JSONPatch** dizisi üretir ve API sunucusuna base64 olarak döner:

```yaml
# Mutating webhook tanımındaki temel ayrım:
apiVersion: admissionregistration.k8s.io/v1
kind: MutatingWebhookConfiguration
metadata:
  name: sidecar-injector-webhook
webhooks:
  - name: "injector.company.com"
    clientConfig:
      service:
        namespace: security-tools
        name: injector-service
        path: "/mutate"              # Mutation mantığını yürüten endpoint
    rules:
      - operations: ["CREATE"]
        resources: ["pods"]
# Webhook servisinin API sunucusuna döndüğü JSONPatch mantığı:
# [{"op": "add", "path": "/spec/containers/-", "value": {"name": "sidecar", "image": "envoy:v1.30"}}]
```
...
Kubernetes 1.30+ (2026 Standardı) ile birlikte GA aşamasına geçen **ValidatingAdmissionPolicy**, basit doğrulama kuralları için harici bir web sunucusu (webhook) yazma ve yönetme zorunluluğunu ortadan kaldırır. **Common Expression Language (CEL)** kullanılarak doğrudan YAML içinde API Server düzeyinde yüksek performanslı kurallar tanımlanır:
...
spec:
  ...
  validations:
    - expression: "object.spec.containers.all(c, !c.image.endsWith(':latest'))"
      message: "Konteyner imajlarında 'latest' etiketi kullanılamaz!"
```
...
```bash
# Kyverno'nun otomatik oluşturduğu webhook'ları inceleyin
kubectl get validatingwebhookconfigurations
# Çıktı: kyverno-resource-validating-webhook-cfg

kubectl get mutatingwebhookconfigurations
# Çıktı: kyverno-resource-mutating-webhook-cfg
```
