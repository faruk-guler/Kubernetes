...
```yaml
# GitHub Actions Tedarik Zinciri Adımları (Build -> Scan -> Keyless Sign):
steps:
  ...
  - name: Run Trivy vulnerability scan
    ...
    with:
      ...
      exit-code: '1' # Kritik açık bulunursa pipeline'ı kır
...
## 6. Kyverno ile Tedarik Zinciri Politika Doğrulama

Kümeye imzasız veya yetkilendirilmemiş imajların girmesini engellemek amacıyla Kyverno üzerinde imza doğrulaması (`verifyImages`) ve registry kısıtlaması (`validate`) kuralları birlikte tanımlanabilir:
...
spec:
  ...
  rules:
    # 1. Keyless Cosign Doğrulaması (Sadece güvenilir CI ortamının imzaladığı imajlar kabul edilir)
    - name: verify-keyless-signature
      match:
        resources:
          kinds: ["Pod"]
      verifyImages:
        - imageReferences: ["ghcr.io/my-company/*"]
          attestors:
            - entries:
                - keyless:
                    issuer: "https://token.actions.githubusercontent.com"
                    subject: "https://github.com/my-company/*"

    # 2. Onaylı Kayıt Defteri ve 'latest' Yasağı
    - name: restrict-registry-and-latest
      match:
        resources:
          kinds: ["Pod"]
      validate:
        message: "İmajlar sadece resmi repo üzerinden çekilmeli ve 'latest' etiketi kullanılmamalıdır."
        pattern:
          spec:
            containers:
              - image: "ghcr.io/my-company/*:!latest"
```
