...
Aşağıdaki örnekte replika üst sınırını dinamik belirleyen parametrik bir politika, parametre kaynağı ve bağlama tanımı gösterilmiştir:
...
spec:
  ...
  validations:
    - expression: "object.spec.replicas <= int(params.data.maxReplicas)"
      messageExpression: "'Replika sayısı izin verilen üst sınırı aşıyor: ' + params.data.maxReplicas"
```
...
### 3. Bağlama Tanımı (`ValidatingAdmissionPolicyBinding`)
