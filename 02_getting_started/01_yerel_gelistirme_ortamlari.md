...
| Özellik / Araç | Minikube | Kind (K8s in Docker) | K3d (Rancher K3s) | Docker Desktop K8s | MicroK8s (Canonical) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Mimari Yaklaşım** | Tek düğümlü VM veya Konteyner temelli. | K8s bileşenlerini Docker konteynerleri içinde çalıştırır. | K3s'in Docker içinde paketlenmiş ultra hafif halidir. | Docker Desktop uygulamasıyla gelen entegre K8s. | Ubuntu Snap tabanlı, izole ve doğrudan OS üzerinde koşan hafif K8s. |
| **Çoklu Düğüm (Multi-Node)** | Destekler ancak hantaldır. | Doğal olarak destekler. CI/CD ortamları için harikadır. | Çoklu düğümü en hızlı ve en düşük RAM ile ayağa kaldıran araçtır. | Desteklemez (Tek node). | `microk8s add-node` ile çok kolay çoklu düğüm desteği. |
| **Kaynak Tüketimi (RAM/CPU)** | Görece yüksektir. | Orta seviyededir. | En düşük (En hafif) olanıdır. | Docker Desktop tüketimine eklendiği için yüksektir. | Düşük / Orta (Kernel seviyesinde optimize). |
| **Öne Çıkan Özellik (Addon)** | `minikube addons` ile hazır servisler. | Saf upstream K8s testleri ve CNCF e2e uyumu. | Port yönlendirme ve saniyeler içinde ayağa kalkma. | Arayüzden tek tıkla "Enable Kubernetes" kolaylığı. | `microk8s enable ingress dns metallb` komutlarıyla üretim/edge hazırlığı. |
...
## 🛠️ Pratik Otomasyon: Tek Komutla Çok Düğümlü Yerel Laboratuvar Kurulumu

Kitap içerisindeki tüm laboratuvar senaryolarını (Ingress, MetalLB, çoklu düğüm, NetworkPolicy) eksiksiz çalıştırabileceğiniz 1 Control Plane ve 2 Worker Node'lu hazır Kind kümesini aşağıdaki betikle saniyeler içinde kurabilirsiniz:

```bash
# Yerel test kümesini otomatik olarak ayağa kaldırın:
bash ../scripts/setup_local_lab.sh
```

> [!TIP]
> Bu betik [`scripts/setup_local_lab.sh`](../scripts/setup_local_lab.sh) dosyasında yer alır. Kind konfigürasyonunu otomatik üretir, port yönlendirmelerini (80 ve 443) ayarlar ve kubectl context'ini hazırlar.
