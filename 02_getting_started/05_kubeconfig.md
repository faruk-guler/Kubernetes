...
Aşağıda, çoklu küme ve kullanıcı tanımlarını (Client Certificate ve OIDC kimlik doğrulama dahil) içeren üretim standardında bir Kubeconfig manifestosu yer almaktadır:
...
# 1. Kümeler (API Server adresleri ve güvenilen CA sertifikaları)
clusters:
  - name: production-cluster
    cluster:
      server: https://k8s.company.internal:6443
      certificate-authority-data: <base64-ca-cert>

# 2. Kullanıcılar (Kimlik doğrulama anahtarları, sertifikalar veya token'lar)
users:
  - name: developer-user
    user:
...
# 3. Bağlamlar (Kullanıcı + Küme + Varsayılan Namespace eşleştirmesi)
contexts:
  - name: dev-context
    context:
...
kubectl config set-credentials developer-user --token="<BEARER_TOKEN>"
...
## 🛠️ Pratik Otomasyon: Kısıtlı Geliştirici Kubeconfig Dosyası Üretme

Bir geliştiriciye küme admin şifresini vermek yerine, yalnızca belirli bir namespace üzerinde işlem yapabileceği TLS istemci sertifikası üretip tek dosyada paketleyen otomasyon betiğini kullanabilirsiniz:

```bash
# Kullanım: bash ../scripts/create_developer_kubeconfig.sh <kullanıcı_adı> <namespace>
bash ../scripts/create_developer_kubeconfig.sh ahmet development
```

> [!TIP]
> Bu betik [`scripts/create_developer_kubeconfig.sh`](../scripts/create_developer_kubeconfig.sh) dosyasında yer alır. OpenSSL ile özel anahtar ve CSR oluşturur, Kubernetes CertificateSigningRequest API'si ile onaylar, namespace bazlı RBAC rolü tanımlar ve taşınabilir bağımsız bir `ahmet-development.kubeconfig` dosyası üretir.
