...
Gateway API ile trafiği iki farklı servis sürümü arasında yüzdesel olarak paylaştırmak için `HTTPRoute` kuralı altındaki `backendRefs` listesine `weight` (ağırlık) parametresi eklenir:
