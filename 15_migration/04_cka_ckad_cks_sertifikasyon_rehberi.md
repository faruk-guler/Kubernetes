...
Sadece `tier: frontend` etiketine sahip podlardan `tier: backend` podlarının 80 portuna gelen trafiğe izin veren NetworkPolicy:

```yaml
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
  name: cka-allow-ingress-backend
  namespace: default
spec:
  podSelector:
    matchLabels:
...
Ana konteynerin logunu ortak `emptyDir` birimi üzerinden okuyup ekrana yazan yan (sidecar) konteyner pod kurgusu sınavda sıklıkla talep edilir:

```yaml
apiVersion: v1
kind: Pod
metadata:
  name: cka-multi-container-pod
  namespace: default
  labels:
    tier: backend
    app: secure-app
