...
kubectl describe pod nginx-pod | grep -A 20 Conditions

# JSON formatında
kubectl get pod nginx-pod -o jsonpath='{.status.conditions[*].type}'
```
...
kubectl logs nginx-pod --previous          # Önceki crash logu
kubectl describe pod nginx-pod             # Events ve restart sayısı
```
