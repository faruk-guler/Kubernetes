...
kubectl debug -it nginx-pod --image=nicolaka/netshoot --target=<ana-konteyner>
...
kubectl debug -it nginx-pod --image=busybox:1.36 --target=<konteyner>

# Pod'u kopyalayarak debug (orijinali bozmadan)
kubectl debug nginx-pod -it --image=ubuntu --copy-to=debug-pod --share-processes
```
