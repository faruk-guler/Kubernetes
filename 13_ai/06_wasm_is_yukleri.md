...
metadata:
  name: wasm-spin
handler: spin # config.toml içindeki spin shim adını referans alır
  ...
  template:
    metadata:
      labels:
        app: wasm-hello
    spec:
      runtimeClassName: wasm-spin # WASM Shim'i tetikleyecek sınıf
      containers:
      - name: wasm-app
        image: ghcr.io/spinkube/containerd-shim-spin/examples/spin-rust-hello:v0.13.0
        command: ["/"] # Spin başlangıç noktası
        ports:
        - containerPort: 80
```
...
kind: Service
metadata:
  name: wasm-serverless-api
  namespace: default
spec:
  template:
    metadata:
      annotations:
        autoscaling.knative.dev/min-scale: "0" # İstek gelmediğinde sıfır kaynak tüketimi
    spec:
      runtimeClassName: wasm-spin
      containers:
        - image: ghcr.io/my-company/k8s-hello-wasm:v1.0
          command: ["/"]
          ports:
            - containerPort: 80
```
