...
NVIDIA GPU Operator altında bir ConfigMap oluşturularak tek bir GPU'nun yazılımsal olarak birden fazla sanal GPU'ya (time-slicing) bölüneceği bildirilir.
...
Bir podun içinden GPU kullanabilmek için `resources.limits` altına `nvidia.com/gpu` veya bölünmüş GPU kaynağı eklenir:

```yaml
apiVersion: v1
kind: Pod
metadata:
  name: gpu-time-slice-pytorch-pod
  namespace: ai-workloads
spec:
  containers:
    - name: pytorch-inference
      image: pytorch/pytorch:2.4.0-cuda12.4-cudnn9-runtime
      command: ["python", "-c", "import torch; print('CUDA Ready:', torch.cuda.is_available())"]
      resources:
        limits:
          nvidia.com/gpu: "1" # Time-slicing ile 1 GPU birden fazla pod tarafından paylaşılır
```
