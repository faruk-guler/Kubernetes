...
spec:
  clusterIP: None # Headless servis: Tek bir VIP yerine doğrudan pod IP'lerine çözer
  selector:
    app: mysql
  ports:
    - port: 3306
...
`<pod-adi>.<headless-servis-adi>.<namespace>.svc.cluster.local` (Örn: `mysql-cluster-0.mysql-headless.production.svc.cluster.local`).
