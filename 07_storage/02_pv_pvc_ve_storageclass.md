...
### Dinamik Sağlama Örneği (AWS EBS CSI)

Bulut ortamlarında dinamik provizyon sağlamak için `provisioner: ebs.csi.aws.com` tanımlı ve `volumeBindingMode: WaitForFirstConsumer` parametresiyle pod'un zamanlandığı kullanılabilirlik alanında (Availability Zone) disk oluşturan bir StorageClass şablonu yaygın olarak tercih edilir:
...
volumeBindingMode: WaitForFirstConsumer # Pod düğüme planlanana kadar disk oluşturmayı bekle (AZ uyumu)
