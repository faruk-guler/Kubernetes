...
* **Özetle:** Alev grafiğine baktığınızda, yatayda en geniş yer kaplayan kod blocksunu bulup optimize ederseniz, bulut faturanızdaki sunucu maliyetlerini anında yarı yarıya düşürebilirsiniz. FinOps ve Profiling 2026'nın ayrılmaz bir ikilisidir.

---

## 🛠️ Pratik Otomasyon: Pod İçi Go (pprof) ve Java (JFR / Heap) Profil Yakalama

Kümedeki belirli bir Go veya Java podunda ani bellek şişmesi veya CPU kilitlemesi yaşandığında, hızlıca teşhis verisi toplamak için aşağıdaki betiği kullanabilirsiniz:

```bash
# Kullanım: bash ../scripts/capture_java_go_pprof.sh <pod_adı> [namespace] [dil: go|java]
bash ../scripts/capture_java_go_pprof.sh backend-api-7b8f9c-xyz production go
```

> [!TIP]
> Bu betik [`scripts/capture_java_go_pprof.sh`](../scripts/capture_java_go_pprof.sh) dosyasında yer alır. Go uygulamaları için HTTP pprof uç noktalarından (`/debug/pprof/profile` ve `/debug/pprof/heap`) anlık döküm çeker; Java uygulamaları içinse `jcmd` ve `jstack` komutlarıyla thread dump ve heap analizi üretip yerel diskinize indirir.
