...
Konteyner içinden `/etc/shadow` veya `/etc/passwd` gibi kritik sistem dosyalarının okunma girişimlerini kernel seviyesinde yakalayan kprobe tanımı:
...
### C. Yetki Yükseltme (Privilege Escalation) Tespiti ve Engelleme

Konteyner içindeki bir sürecin root hakları kazanma girişimini (`commit_creds` kernel fonksiyonu) anında tespit edip süreci `Sigkill` ile durduran kural:
