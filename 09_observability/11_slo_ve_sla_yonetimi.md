...
spec:
  target: "99.9"
  window: 30d
  description: "Billing API isteklerinin %99.9'u hatasiz (2xx/3xx) donmelidir."
  indicator:
    ratio:
      errors:
        metric: http_requests_total{status=~"5..", job="billing-api"}
      total:
        metric: http_requests_total{job="billing-api"}
```
