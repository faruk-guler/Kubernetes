...
spec:
  ...
  maxUnhealthy: 40% # Aynı anda en fazla %40 düğüm onarıma alınabilir
  nodeStartupTimeout: 10m # Yeni node 10 dakikada hazır olmazsa sil ve yeniden dene
  selector:
    matchLabels:
      cluster.x-k8s.io/cluster-name: my-workload-cluster
  unhealthyConditions:
  - type: Ready
    status: Unknown
    timeout: 300s
...
spec:
  topology:
    class: my-standard-cluster-class # Önceden platform ekibince tanımlanmış sınıf
    version: v1.30.0
    controlPlane:
      replicas: 3
    workers:
