...
Git reposundaki kodların hangi etiketlere (labels) sahip kümelere dağıtılacağını belirleyen bir GitRepo kaynağı tanımlanır.
...
spec:
  repo: https://github.com/my-company/store-apps
  branch: main
  paths:
  - manifests/pos-system # Git içindeki spesifik klasör
  targets:
  - clusterSelector:
      matchLabels:
        env: edge-store # Sadece bu etikete sahip olan kümelere bas
```
...
- name: store-istanbul
  ...
  helm:
    values:
      posAPI:
        storeName: "Istanbul Merkez"
        localDbIP: "192.168.1.100"
- name: all-eu-stores
  clusterGroup: eu-west-stores
  helm:
    values:
      currency: "EUR"
```
