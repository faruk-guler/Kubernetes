...
sync:
  nodes:
    enabled: true
    syncAllNodes: true # Host üzerindeki tüm düğümleri sanal olarak göster
  ingressclasses:
    enabled: true # Host kümedeki Ingress nesnelerini kullanıma aç
  storageclasses:
    enabled: true # Host'un depolama altyapısını kullan
vcluster:
    ...
    - name: Create Ephemeral vCluster
      run: |
        vcluster create pr-${{ github.event.pull_request.number }} \
          -n vcluster-pr-${{ github.event.pull_request.number }} \
          --connect=false
    - name: Run E2E Tests
      run: |
        vcluster connect pr-${{ github.event.pull_request.number }} -n vcluster-pr-${{ github.event.pull_request.number }}
        export KUBECONFIG=./kubeconfig.yaml
        helm test my-app
    - name: Cleanup vCluster
      if: always()
      run: |
        vcluster delete pr-${{ github.event.pull_request.number }} -n vcluster-pr-${{ github.event.pull_request.number }}
```
