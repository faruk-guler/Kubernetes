...
spec:
  bootstrapServers:
    - my-kafka-cluster.kafka.svc:9092
  topics:
    - orders-topic
...
spec:
  delivery:
    retry: 5 # 5 kez yeniden dene
    backoffPolicy: exponential
    backoffDelay: "PT1S" # 1 saniye arayla (katlanarak artar)
    deadLetterSink:
      ref:
        apiVersion: serving.knative.dev/v1
        kind: Service
        name: dlq-logger-service # Hatalı mesajlar buraya atılır
```
