...
spec:
  ...
  source:
    ...
    targetRevision: HEAD
    path: bootstrap/clusters/production
  destination:
...
### Prod Ortamı İçin Yapılandırma Farkı (`overlays/prod/argocd-app-prod.yaml`)

Ortam bazlı Application tanımlarında temel şablon aynı kalırken, sadece hedef Git yolu (`path`) ve hedef Kubernetes ad alanı (`namespace`) özelleştirilir:

```yaml
spec:
  project: default
  source:
    repoURL: 'https://github.com/company/my-app-gitops.git'
    targetRevision: HEAD
    path: overlays/prod          # Hedeflenen ortam dizini
  destination:
    server: 'https://kubernetes.default.svc'
    namespace: production        # Prod ortamı ad alanı
```
...
*(Detaylı örnekler ve kurallar için bkz: ArgoCD ApplicationSet)*
