...
spec:
  ...
  strategy:
    type: RollingUpdate
    rollingUpdate:
      maxSurge: 1        # Güncelleme anında en fazla 1 ek pod oluştur (Kapasite + 1)
      maxUnavailable: 0  # Güncelleme sırasında daima 5 pod hazır kalsın (sıfır kesinti)
  selector:
    matchLabels:
      app: billing-api
  template:
    ...
    spec:
      containers:
        - name: billing-api
          image: registry.company.com/billing-api:v2.1.0
```
...
### Recreate Stratejisi Yapılandırması
...
*(Detaylar ve örnek yapılandırmalar için bkz: Argo Rollouts)*
