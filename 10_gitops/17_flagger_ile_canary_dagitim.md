...
spec:
  ...
  # Otomatik ölçekleyici (HPA) referansı
  autoscalerRef:
  ...
  # Aşamalı Dağıtım Kuralları
  analysis:
    interval: 1m      # Her adım arasında 1 dakika bekle ve ölçüm yap
    threshold: 5      # 5 kez üst üste başarısız olursa DERHAL geri al (rollback)
    maxWeight: 50     # Trafik en fazla %50'ye kadar bölünsün
    stepWeight: 10    # Her başarılı adımda trafiği %10 artır (10% -> 20% -> 30%...)
    
    # Doğrulanacak Metrikler
    metrics:
      ...
      - name: request-duration
        thresholdRange:
          max: 500    # P99 yanıt süresi 500 milisaniyeyi geçmemelidir
        interval: 1m
