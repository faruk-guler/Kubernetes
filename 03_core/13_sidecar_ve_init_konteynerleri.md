...
## 6. Native Sidecar ile Job Entegrasyonu

Native sidecar özelliğinin en güçlü yanlarından biri Job ve CronJob iş yükleridir. Bir Job nesnesinde `initContainers` altına `restartPolicy: Always` niteliğinde yardımcı bir servis (örneğin log toplayıcı veya proxy) eklendiğinde; ana iş yükü tamamlandığında sidecar konteyner de otomatik olarak sonlandırılır ve pod başarıyla `Completed` aşamasına geçer.
