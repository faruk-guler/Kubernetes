...
Varsayılan olarak, bir pod içerisindeki konteynerler birbirinin süreçlerini (processes) göremez. Ancak sorun giderme veya ortak işlem yürütme amacıyla pod seviyesinde `shareProcessNamespace: true` parametresi aktif edilebilir:

```yaml
apiVersion: v1
kind: Pod
metadata:
  name: process-sharing-pod
spec:
...
## 4. Pod Önceliklendirme ve Tahliye (Pod Priority & Preemption)

Kaynakların kısıtlı olduğu yoğun küme ortamlarında, kritik üretim iş yüklerinin (örn: faturalama API'si veya veritabanı) ayakta kalmasını; gerekirse daha az önemli arka plan görevlerinin geçici olarak sonlandırılmasını (tahliye edilmesini / preempt) sağlamak için **PriorityClass** kullanılır.
...
description: "Kritik iş yükleri için yüksek öncelik sınıfı."
```

### Pod İçinde PriorityClass Kullanımı
...
* **Preemption Mantığı:** Düğümde yeterli CPU veya bellek kalmadığında, `kube-scheduler` düşük öncelikli pod'ları düğümden tahliye eder (`evict`) ve yüksek öncelikli pod'un ihtiyaç duyduğu kaynakları derhal tahsis eder.

---

## 5. Konteyner Türleri Karşılaştırma Matrisi
