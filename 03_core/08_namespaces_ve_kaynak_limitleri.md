# Namespaces, Etiketler ve Kaynak Limitleri

Kubernetes kümesi içerisinde nesneleri (objects) organize ederken ilk adımımız, bu devasa yapıyı yönetilebilir havuzlara bölmek (Namespace) ve iş yüklerimizi doğru etiketlerle işaretlemektir (Labels).
...
* `default`: Namespace belirtmeden oluşturulan tüm kullanıcı iş yüklerinin düştüğü varsayılan alan.
* `kube-system`: Kubernetes'in kendi kontrol düzlemi ve sistem podlarının (CoreDNS, kube-proxy, CNI vb.) çalıştığı kritik alan.
* `kube-public`: Kimlik doğrulaması gerektirmeyen, küme genelinde tüm istemcilerce okunabilen kamuya açık veriler (örn: `cluster-info` ConfigMap) için kullanılır.
* `kube-node-lease`: Her düğümün (node) kontrol düzlemine periyodik kalp atışı (heartbeat / lease) göndererek sağlıklı olduğunu bildirdiği, API sunucusu üzerindeki yükü azaltan özel alan.
...
> [!TIP]
> Pratik bir yöntem olarak `kubens` (veya `krew install ns`) eklentisiyle bu işlemi `kubectl ns team-alpha` yazarak saniyeler içinde yapabilirsiniz.
