...
Ekiplerin kontrolsüzce büyük kaynaklar tüketmesini engellemek için isim alanlarına ResourceQuota ve LimitRange nesneleriyle sınırlar getirilir.
...
## 🛠️ Pratik Otomasyon: Kümedeki Kaynak İsrafını ve Limitsiz Pod'ları Raporlama

Kümenizde requests/limits atanmamış tehlikeli pod'ları ve rezerve edilip kullanılmayan atıl CPU/RAM kaynaklarını hızlıca listelemek için aşağıdaki FinOps denetim betiğini kullanabilirsiniz:

```bash
bash ../scripts/report_resource_waste.sh
```

> [!TIP]
> Bu betik [`scripts/report_resource_waste.sh`](../scripts/report_resource_waste.sh) dosyasında yer alır. `kubectl top` ve pod metriklerini karşılaştırarak fazla tahsis edilmiş (overprovisioned) konteynerleri ve limitsiz çalışan iş yüklerini anında listeler.
