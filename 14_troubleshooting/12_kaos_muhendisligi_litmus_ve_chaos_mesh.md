...
Aşağıdaki deney, `payment-service` etiketli Pod'lara **200ms'lik bir ağ gecikmesi** enjekte eder. Deney süresi dolduğunda kurallar otomatik temizlenir ve trafik normale döner:
...
spec:
  ...
  delay:
    latency: "200ms"
    jitter: "20ms"
  duration: "5m" # Deney 5 dakika sonra otomatik temizlenir
```

Bu YAML'ı Kubernetes kümenize `kubectl apply` ile uyguladığınız anda, eBPF ve tc (Traffic Control) kuralları devreye girer ve ilgili pod'un ağ paketlerini kasıtlı olarak bekletmeye başlar.
