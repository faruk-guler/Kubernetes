...
## 🛠️ Pratik Otomasyon: Kümeden Temizlenmiş (Production-Ready) YAML Dışa Aktarma

Velero haricinde veya hızlı bir manifesto yedeği gerektiğinde, kümedeki kaynakları `status`, `uid`, `resourceVersion` ve `managedFields` gibi gürültülü alanlardan arındırılmış temiz YAML olarak dışa aktarmak için aşağıdaki betiği kullanabilirsiniz:

```bash
# Kullanım: bash ../scripts/export_clean_cluster_yamls.sh [hedef_dizin] [namespace]
bash ../scripts/export_clean_cluster_yamls.sh ./backup-yamls production
```

> [!TIP]
> Bu betik [`scripts/export_clean_cluster_yamls.sh`](../scripts/export_clean_cluster_yamls.sh) dosyasında yer alır. `kubectl-neat` ve `yq` kullanarak tüm Deployment, Service, ConfigMap ve Secret nesnelerini başka bir kümeye doğrudan `kubectl apply` ile uygulanabilecek temizlikte arşivler.
