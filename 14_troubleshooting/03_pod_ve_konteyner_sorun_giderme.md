...
## 🛠️ Pratik Otomasyon: Pod Çökme Teşhisi ve Askıda Kalan Kaynak Temizliği

### 1. Çöken Pod Otomatik Analiz Aracı

CrashLoopBackOff, OOMKilled veya ImagePullBackOff yaşayan pod'ların önceki loglarını (`--previous`), çıkış kodlarını (exit code) ve OOM bellek limitlerini tek komutla analiz etmek için:

```bash
# Kullanım: bash ../scripts/diagnose_pod_crashes.sh <pod_adı> [namespace]
bash ../scripts/diagnose_pod_crashes.sh broken-app-6d847c-xyz default
```

> [!TIP]
> Bu betik [`scripts/diagnose_pod_crashes.sh`](../scripts/diagnose_pod_crashes.sh) dosyasında yer alır ve yaygın hata kodlarını (Exit Code 137 OOM, 139 Segfault, 127 Command Not Found) otomatik yorumlar.

### 2. Terminating Durumunda Askıda Kalan Kaynakları Temizleme

Finalizer kilitlenmesi veya silinmeyen Namespace / Pod / PVC nesnelerini temizlemek için:

```bash
# Kullanım: bash ../scripts/cleanup_stuck_resources.sh <kaynak_türü> <kaynak_adı> [namespace]
bash ../scripts/cleanup_stuck_resources.sh pod stuck-pod-xyz default
```

> [!TIP]
> Bu betik [`scripts/cleanup_stuck_resources.sh`](../scripts/cleanup_stuck_resources.sh) dosyasında yer alır; finalizer listesini güvenle sıfırlar ve kaynak blokajını kaldırır.
