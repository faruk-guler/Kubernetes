...
## 🛠️ Pratik Otomasyon: Düğüm Bakım ve Küme Sağlık Denetimi

### 1. Hızlı Düğüm Bakım Betiği (Cordon / Drain / Uncordon)

Düğümleri güvenle bakıma almak, DaemonSet ve local storage podlarını koruyarak tahliye etmek ve bakım sonrası geri açmak için:

```bash
# Bakıma al (Drain):
bash ../scripts/quick_node_maintenance.sh drain worker-node-01

# Bakımdan çıkar (Uncordon):
bash ../scripts/quick_node_maintenance.sh uncordon worker-node-01
```

> [!TIP]
> Bu betik [`scripts/quick_node_maintenance.sh`](../scripts/quick_node_maintenance.sh) dosyasında yer alır. Pod tahliye sürecini adım adım raporlar ve PDB engellerini güvenle yönetir.

### 2. Kapsamlı Küme Sağlık Taraması (Health Check)

Tüm düğümlerin Ready durumu, kontrol düzlemi bileşenleri, etcd yanıt süresi, CoreDNS ve pod restart oranlarını tek raporda toplamak için:

```bash
bash ../scripts/cluster_health_check.sh
```

> [!TIP]
> Bu betik [`scripts/cluster_health_check.sh`](../scripts/cluster_health_check.sh) dosyasında yer alır; yeşil/sarı/kırmızı göstergelerle kümenin genel sıhhatini özetler.
