...
## 🛠️ Pratik Otomasyon: Depolama Denetimi ve Veri Kurtarma

### 1. Kullanılmayan ve Boşa Para Harcatan Atıl PVC'leri Bulma

Kümede hiçbir Pod tarafından mount edilmemiş, unutulmuş ve gereksiz maliyet oluşturan PVC'leri listelemek için:

```bash
bash ../scripts/audit_unused_pvc_storage.sh
```

> [!TIP]
> Bu betik [`scripts/audit_unused_pvc_storage.sh`](../scripts/audit_unused_pvc_storage.sh) dosyasında yer alır. Tüm namespace'lerdeki PVC'leri tarar, aktif pod bağlaması olmayanları gigabyte ve maliyet riskiyle listeler.

### 2. Canlı PVC Verisini Yerel Makineye Yedekleme (PVC Backup to Local)

Kritik bir PVC içindeki verileri geçici bir pod açarak sıkıştırılmış `.tar.gz` arşivi olarak yerel bilgisayarınıza indirmek için:

```bash
# Kullanım: bash ../scripts/backup_pvc_to_local.sh <pvc_adı> [namespace] [hedef_dosya.tar.gz]
bash ../scripts/backup_pvc_to_local.sh data-postgres-0 database ./postgres-data.tar.gz
```

> [!TIP]
> Bu betik [`scripts/backup_pvc_to_local.sh`](../scripts/backup_pvc_to_local.sh) dosyasında yer alır; `kubectl exec` ve `tar` boru hattı kullanarak PVC'deki tüm dosyaları canlı olarak yerel diskinize kopyalar.
