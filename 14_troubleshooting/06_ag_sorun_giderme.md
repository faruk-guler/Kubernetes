...
## 🛠️ Pratik Otomasyon: Ağ Bağlantı ve Paket Yakalama Araçları

### 1. Pod'dan Hedefe Ağ Erişilebilirlik Testi

Belirli bir hedefe HTTP/HTTPS/TCP üzerinden curl atarak gecikme süresini, HTTP durum kodunu ve DNS çözümlemesini test eden geçici teşhis podu:

```bash
# Kullanım: bash ../scripts/test_pod_connectivity.sh <hedef_url_veya_ip> [port]
bash ../scripts/test_pod_connectivity.sh payment-service.production.svc.cluster.local 8080
```

> [!TIP]
> Bu betik [`scripts/test_pod_connectivity.sh`](../scripts/test_pod_connectivity.sh) dosyasında yer alır; `curlimages/curl` imajı ile tek seferlik güvenli pod açıp otomatik temizler.

### 2. Canlı Pod Üzerinde Paket Yakalama (tcpdump Trace)

Pod seviyesinde giren/çıkan paketleri Wireshark formatında (`.pcap`) yakalayıp yerel bilgisayarınıza indirmek için:

```bash
# Kullanım: bash ../scripts/capture_pod_packet_trace.sh <pod_adı> [namespace] [süre_saniye]
bash ../scripts/capture_pod_packet_trace.sh frontend-6f97d-xyz production 30
```

> [!TIP]
> Bu betik [`scripts/capture_pod_packet_trace.sh`](../scripts/capture_pod_packet_trace.sh) dosyasında yer alır. Ephemeral debug container ile pod ağına sızar ve `.pcap` dosyasını yerel makinenize çeker.

### 3. CoreDNS Hızlı Hata Ayıklama

Cluster içi CoreDNS sunucularının sağlık durumunu, loglarını ve domain sorgularını anında test etmek için:

```bash
bash ../scripts/debug_dns.sh
```

> [!TIP]
> Bu betik [`scripts/debug_dns.sh`](../scripts/debug_dns.sh) dosyasında yer alır. CoreDNS Pod'larının loglarını, upstream DNS yapılandırmasını ve çözümleme sürelerini ekrana döküm olarak basar.
