...
GitLab, hem CI/CD hem de Git deposunu tek platformda sunar. Kubernetes entegrasyonu Agent (Pull tabanlý) veya Runner üzerinden saðlanýr.

---

## GitLab Agent for Kubernetes (KAS)

GitLab'ýn modern Kubernetes entegrasyon yöntemi. Push deðil Pull-based — cluster güvenlik duvarý arkasýnda olsa bile çalýþýr.

```bash
# GitLab UI › Infrastructure › Kubernetes clusters › Connect a cluster
# Verilen komutu cluster'da çalýþtýr:
...
ci_access:
  projects:
  - id: company/backend-api         # Bu CI job'larý cluster'a eriþebilir
  - id: company/frontend-app
  groups:
  - id: company/microservices       # Bu grup altýndaki tüm projeler
```
  ...
  script:
    - |
      ...
      kubectl rollout status deployment/api \
        -n staging \
        --timeout=5m
    - echo "Staging deploy baþarýlý: $IMAGE_TAG"
  rules:
    - if: $CI_COMMIT_BRANCH == "main"
...
deploy-production:
  ...
  script:
    - |
      ...
      # GitLab Agent bu deðiþikliði algýlayýp sync eder
...
```yaml
# Ortam listesi ve geçmiþi GitLab UI'da görünür
...
> [!TIP]
> GitLab'ýn **Merge Request Pipeline** özelliðini kullanýn — sadece main'e merge olan kod production'a gider, her PR kendi staging ortamýnda test edilir.
