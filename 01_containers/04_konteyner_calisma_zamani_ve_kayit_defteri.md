...
## 5. Registry Öncesi Otomatik Zafiyet Taraması

İmajları derleyip Harbor veya harici bir kayıt defterine göndermeden önce CI/CD hattı üzerinde otomatik güvenlik taraması yürütülmelidir:

```bash
# Harbor'a push etmeden önce kritik CVE taraması (CI Pipeline güvenlik kapısı):
trivy image --exit-code 1 --severity CRITICAL,HIGH myapp:v1.0.0
```

Eğer imajda kritik seviyede bir güvenlik açığı bulunursa `exit-code 1` döner ve pipeline durdurularak güvensiz imajın registry'ye yüklenmesi engellenir.

*(Trivy, Syft ile SBOM üretimi ve Cosign dijital imzalama pratikleri için bkz: **Bölüm 06 — İmaj Güvenliği, Tarama ve SBOM**).*
