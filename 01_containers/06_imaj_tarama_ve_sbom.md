...
## 5. Kubernetes Kabul Denetimi (Admission Control) ile Doğrulama

CI/CD hattında güvenlik taramasından geçirilen ve Cosign ile dijital olarak imzalanan konteyner imajları, Kubernetes kümesine dağıtılırken API Server kapısında doğrulanır:

* **Kabul Denetimi Mantığı (Admission Control):** API sunucusu, bir pod oluşturma isteği aldığında nesneyi veritabanına (etcd) kaydetmeden önce politika denetleyicisine (Kyverno veya OPA Gatekeeper) yönlendirir.
* **İmza ve İlke Doğrulama:** İmajın onaylı registry'den gelip gelmediği ve geçerli bir açık anahtarla (`cosign.pub`) imzalanıp imzalanmadığı kontrol edilir. Doğrulamadan geçemeyen imajlar kümede **kesinlikle başlatılmaz**.

*(Kubernetes üzerinde Kyverno politikalarının yapılandırılması, Cosign açık anahtar entegrasyonu ve tam imza doğrulama kuralları için bkz: **Bölüm 08 — Güvenlik / 14 — Tedarik Zinciri Güvenliği**).*
